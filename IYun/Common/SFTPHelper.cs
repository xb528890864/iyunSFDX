...
namespace Terminal_CDMA
{
    ...
    public class SFTPHelper
    {
        ...
        public SFTPHelper(string ip, string port, string user, params PrivateKeyFile[] keyFiles)
        {
            sftp = new SftpClient(ip, Int32.Parse(port), user, keyFiles);
        }
        ...
        public string GetFileListFirst(string remotePath, string fileSuffix)
        {
            try
            {
                Connect();
                var files = sftp.ListDirectory(remotePath);
                Disconnect();
                var a = files.Where(u=>u.Name.Contains("学生成绩")&&u.Length!=0).OrderByDescending(u => u.LastWriteTime).FirstOrDefault();
